...
void LedStep(enum StepDirection eStepDirection){
	...
	static unsigned int uiStepChange; /* na start obiektowi statycznemu przypisywane jest 0, nie "umiera" po zakonczeniu wywolania funkcji*/
	
	if(LEFT == eStepDirection){
		uiStepChange = (uiStepChange+1)%4;             /*w lewo dodaje*/
	}
	else if(RIGHT == eStepDirection){
		uiStepChange = (uiStepChange-1)%4;             /*w prawo odejmuje*/
	}
	LedOn(uiStepChange);
}
